...
export async function cargarProductos() {
  try {
    ...
    productos = productos.map((producto, index) => ({ ...producto, id: index }))
    carritoID = productos
    return carritoID
  } catch (err) {
    console.log("error al recibir la respuesta del backend", { err: message })
                ...
                <div class="overlay__flex" id="producto-${producto.id}">
                    ...
                    <div class="overlay__tarjeta-2">
                        ...
                        <div class="contenedor__botonera">
                            <button type="button" class=${producto.stock > 0 ? "agregar__boton" : "boton__disabled"} data-id="${producto.id}" ${
        producto.stock === 0 ? "disabled" : ""
      }>${producto.stock < 1 ? "Sin stock" : "Agregar"}</button>
